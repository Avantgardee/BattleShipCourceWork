...
Uses
    ...
    Vcl.ExtCtrls, UnitSound, Vcl.StdCtrls, BattleShipUnit, MainMenuUnit,
    Vcl.Menus;
...
Procedure TAddShipsForm.InitializationResourses();
Var
    I, J: Integer;
Begin
...
Procedure TAddShipsForm.ReverseOrientation();
Begin
...
Procedure TAddShipsForm.FillMapWithShips(X, Y, Naklon: Integer; Var Map: TMap;
  Var CoorArrOfPlayer: TCoordinatesArr);
Var
    IsSetShip: Boolean;
Begin
    ...
    Begin
        ...
        Begin
            Inc(CountOfShips);
            If (Current = 0) And (MenuForm.FIsSound) Then
...
Procedure TAddShipsForm.DrawLettersAndNum(I: Integer);
Begin
...
Procedure TAddShipsForm.DrawMaterials();
Var
    I, J: Integer;
Begin
...
Begin
    ...
    If Key = VK_ESCAPE Then
        Close;
    If (Key = VK_RETURN) And (Mode = 1) And
      Not((Username.Text = '') Or (''.IsNullOrWhiteSpace(Username.Text))) Then
...
Procedure TAddShipsForm.RandomShipSet(Var Map: TMap; Var Coor: TCoordinatesArr);
Var
    I, J, N: Integer;
Begin
    Randomize;
...
Begin
    CanClose := Application.MessageBox
      ('Вы уверены, что хотите закрыть это окно', 'Закрыть?',
...
Begin
    ...
    If (Mode = 0) And (CountOfShips < 11) Then
    // Если режим включен на "расстановку" и количество раставленных кораблей меньше
    Begin
        Case CountOfShips Of
            1: // Если номер корабля равен 1, то присвоить числу палуб 4
                NumberOfDecks := 4;
            2, 3:
                NumberOfDecks := 3;
            // Если номер корабля равен 2-3, то присвоить числу палуб 3
            4, 5, 6:
                NumberOfDecks := 2;
            // Если номер корабля равен 4-6, то присвоить числу палуб 2
            7, 8, 9, 10:
                NumberOfDecks := 1;
            // Если номер корабля равен 7-10, то присвоить числу палуб 1
        End;
        If ShipOrientation = 1 Then
        // Проверка состояния оринтации корабля (горизонтальное/вертикальное)
        Begin
            OrientationY := 1; // Если 1, то вертикальное
            OrientationX := 0;
        End
        Else
        Begin
            OrientationY := 0; // Иначе горизонтальное
            OrientationX := 1;
        End;
        For I := 1 To NumberOfDecks Do
        // прорисовка корабля с выбранным числом палуб
        Begin

...
Begin
    CanClose := Application.MessageBox('Вы уверены, что хотите выйти из боя?' +
      #10#13 + 'Все несохранённые данные будут утеряны.', 'Выйти?',
...
Begin
    IsHasShips := False;
    // Установка начального значения флага для обнаружения кораблей
    For I := 0 To 9 Do
        For J := 0 To 9 Do
            If MapOfShips[I, J] = 3 Then
                // Если на карте есть хоть одна палуба неподбитого корабля, то сменить флаг
                IsHasShips := True;
...
Var
    I, J: Integer;
Begin
    Buf.Canvas.Brush.Color := ClWhite; // присвоение белого цвета фону
    Buf.Canvas.Pen.Style := PsClear; // отключение обводки
    Buf.Canvas.Rectangle(0, 0, 1000, 600);
    // Прорисовка прямоугольника с заданными праметрами и стилями
    For I := 0 To 9 Do
    Begin
        DrawLettersAndNum(I); // Прорисовка буквы и цифры
        For J := 0 To 9 Do
        Begin
            ...
            // Прорисовка всего поля игрока без изменений
            Case OpponentMap[I, J] Of // Прорисовка всего поля противника
                0:
                    Buf.Canvas.Draw(I * 32 + 32 + 500, J * 32 + 32, Img[0]);
                // Если в ячейке 0 (нетронутая клетка), то прорисовка картинки моря
                1:
                    Begin
                        Buf.Canvas.Draw(I * 32 + 32 + 500, J * 32 + 32, Img[2]);
                        // Если в ячейке 1 (клетка со значением "Мимо"), то прорисовка картинки штриховки
                        Buf.Canvas.Draw(I * 32 + 500 + 32, J * 32 + 32, Img[1]);
                        // И картинки со снарядом
                    End;
                2:
                    Buf.Canvas.Draw(I * 32 + 500 + 32, J * 32 + 32, Img[2]);
                // Если в ячейке 2 (клетка, которая штрихуется вокруг убитых кораблей)
                3:
                    Buf.Canvas.Draw(I * 32 + 500 + 32, J * 32 + 32, Img[0]);
                // Если в ячейке 3 (клетка с кораблём), то прорисовка картинки моря
                4:
                    Buf.Canvas.Draw(I * 32 + 500 + 32, J * 32 + 32, Img[4]);
                // Если в ячейке 4 (клетка с подбитым кораблём), то прорисовка подбитого корабля
            End;
            If (Mode = 2) And (OpponentMap[I, J] = 3) Then
                // Если режим игры переключен на состояние "закончена",
                // то прорисовка неподбитых кораблей противника
...
Begin
    // Установка каретки в RichEdit в конец
    RichEdit_MoveTo(BattleForm.RichEdit1, BattleForm.RichEdit1.CaretPos.Y, 0);

    // Установить фокус на RichEdit, если форма не закрыта
    Application.ProcessMessages;
    If BattleForm.Showing Then
        BattleForm.RichEdit1.SetFocus;
    // Выбрать нужную букву для цифры, используя коды таблицы ASCII
    For I := #192 To #201 Do
...
Begin
    // Перебор всех кораблей в переданном массиве кораблей
    For I := 1 To 10 Do
    Begin
        // Если число палуб выбранного корабля равно 0, то заштриховать прямоугольную область вокруг корабля,
        // используя координаты начальной и конечной палубы
            ...
            For M := ArrOfShipsCoor[I].StartOfShip[1] - 1 To ArrOfShipsCoor[I]
              .EndOfShip[1] + 1 Do
                For N := ArrOfShipsCoor[I].StartOfShip[2] - 1 To ArrOfShipsCoor
                  [I].EndOfShip[2] + 1 Do
                    // Проверка на то, не вышли ли координаты за пределы массива, и если ячейка с выбранными координатами
                    // имеет значение "море", то присвоить ей номер 2 для штриховки
...
Begin
    I := 1; // Установка начальных значений для поиска
    IsFind := True;
    IsKilled := False;
    While (I < 11) And (IsFind) Do
    // Пока количество просмотренных кораблей меньше 11
    // и корабль с нужными координатами не найден
    Begin
        // Если координата выстрела попадает в палубу корабля
        ...
        Begin
            // Уменьшение количества неподбитых палуб, а также проверка на то, не убит ли корабль
            If IsShot Then
...
Begin
    // Первая возможная клетка для выстрела - верхняя
    PossibleShots[1, 1] := I;
    PossibleShots[1, 2] := J - 1;
    // Вторая возможная клетка для выстрела - нижняя
    PossibleShots[2, 1] := I;
    PossibleShots[2, 2] := J + 1;
    // Третья возможная клетка для выстрела - правая
    PossibleShots[3, 1] := I + 1;
    PossibleShots[3, 2] := J;
    // Третья возможная клетка для выстрела - левая
...
Begin
    // Установка флага подбитого корабля на ложный
    Status := False;
    // Если режим игры включён на "игра"
    If Mode = 1 Then
    Begin
        // Если пользователь нажал на поле противника во время его хода
        If Current = 1 Then
        Begin
            ShowMessage('Сейчас ход опонента');
            Exit;
        End;
        // Вычсиление клетки карты, по которой попал игрок
        X := (X - 500) Div 32 - 1;
        Y := Y Div 32 - 1;
        // Если выстрел в клетку с кораблём
        If (X >= 0) And (Y >= 0) And (X <= 9) And (Y <= 9) And
          (OpponentMap[X, Y] = 3) Then
        Begin
            // Обновить флаг подбитого корабля
            Status := True;
            // Присвоить значение подбитого корабля данной клетке
            OpponentMap[X, Y] := 4;
            // Определение того, какой был выстрел (ранил или убил)
            IsShipKilled := DefineShip(X, Y, OpponentShips, True);
            // Создание сообщения для лога боя
        ...
        Begin
            ...
            // Присвоить значение промаха данной клетке
            OpponentMap[X, Y] := 1;
            // Переключения флага для стрельбы компьютера
            NewShot := False;
            DrawMaterials();
            // Задержка, для того чтобы компьютер не ходил мгновенно
            Delay(900);
            // Если есть возможность хода
            While Not(NewShot) And IsMapHasShips(PlayerMap) Do
            Begin
                // Переключить режим компьютера на "случайная стрельба"
                If ShootingMode = 1 Then
                Begin
                    I := Random(10);
                    J := Random(10);
                End;
                // Если выстрел компьютера неудачный
                ...
                Begin
                    ...
                    Current := 0;
                    // Присвоение ячейке карты значения "промах" и переключение флага
                    PlayerMap[I, J] := 1;
                    DrawMaterials();
                    NewShot := True;
                    ShadeMaps();
...
Begin
    Case CountOfDecks Of
        // В зависимости от числа палуб, переданных как параметр в данную процедуру,
        // Выбирается размер отступа, который будет сделан сверху. Чем меньше кол-во палуб, тем меньше отступ
        1:
            TopOffset := 3;
        2:
            TopOffset := 5;
    ...
    End;
    SkipSquares := 0;
    // количество квадратов кораблей, которые нужно пропустить, чтобы нарисовать
    // новый корабль
    For I := StartShip To EndShip Do
    // В зависимости от переданных параметров, отрисовываются корабли, начиная
    Begin // c корабля с индексом StartShip, заканчивая кораблём с индексом EndShip
        ImgOfDeck := ImgForStat[1];
        If (LocOfShips[I].LifeDecks = 0) Then
            ImgOfDeck := ImgForStat[2];
        // если все палубы корабля подбиты, то сменить картинку на "убит"
        For J := 1 To CountOfDecks Do // отрисовка всех палуб корабля
...
Begin
    ...
    Path := ExtractFileDir(Application.ExeName);
    // Записать имена игроков
    TempRecord.PlayerName1 := Name1;
    TempRecord.PlayerName2 := Name2;
    // Создать процентную запись о состоянии флота игроков и добавить в запись
    ...
    // Записать победителя партии
    TempRecord.Winner := WinnerName;
    // Посчитать общее время игры с учётом того, была ли загружена игра или нет
    ...
    // Добавить созданную запись о бое в список боёв
    TempList.AddNewLineAboutFight(TempRecord);
    // Обновить содержимое файла новый списком
...
Begin
    ...
    Showmessage(Winner + ' победил! ' + #10#13 + 'Общее время игры: ' +
      ...
      ' мин. ' + IntToStr((GetTickCount - TimeOfStartGame + AddTime)
      Div 1000 Mod 60) + ' сек.');
